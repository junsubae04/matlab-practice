%%데이터 불러오기/ 각 종을 숫자로 표현 1: versicolor, 2: viginica/ 학습데이터와 평가데이터 나누기
clear; close all; clc;
...
Test_data=meas(ts_id, 1:2); %sepal length, width만 가져오기
...
%다수결 투표를 통해 최종 그룹 결정: 그룹이 1또는 2밖에 없으므로
idx1=find(k_group==1);
idx2=find(k_group==2);
...
%final_group=mode(k_group); %mode: 최빈값을 찾는 함수.(mean은 평균, median은
%중앙값, mode는 최빈값)
